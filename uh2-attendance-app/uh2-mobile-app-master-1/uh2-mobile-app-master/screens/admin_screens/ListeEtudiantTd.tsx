...
const ListeEtudiantTdScreen = ({ navigation, route }: Props1 & Props2) => {
    ...
    const filteredEtudiants = Td.etudiants.filter(etudiants =>
        etudiants.etudiant.toLowerCase().includes(searchQuery.toLowerCase())
    );
    return (
        <View style={styles.Background} >
                
                <Text style={{ fontSize: 15, fontWeight: 'bold', marginBottom: 10 ,alignSelf:'flex-start'}}>les étudiants de TD: {Td.name}</Text>
                <TextInput
...
export default ListeEtudiantTdScreen;
